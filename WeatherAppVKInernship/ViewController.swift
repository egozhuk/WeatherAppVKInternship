...
class ViewController: UIViewController, CLLocationManagerDelegate {
    ...
    /* :NOTE: 
     В задании указано что прогноз погоды нужно получить на следующие 7 дней, но у меня реализован прогноз погоды только на 5 следующих дней.
     Это связано с тем что бесплатная версия openweathermap API предоставляет прогноз только на 5 дней. Чтобы получить прогноз на неделю нужна платная подписка.
     Я думаю очевидно продемонстрировано мое умение работать с сетевыми запросами json файлами.
     Поэтому реализована поддержка только 5 дней.
    */
